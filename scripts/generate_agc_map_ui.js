...
var generateChart = function (coords) {
  ...
  var szaCollection = images.filter(ee.Filter.calendarRange(9, 12, 'month')).map(function(image){return image.addBands([image.metadata('SOLAR_ZENITH_ANGLE'), image.metadata('SOLAR_ZENITH_ANGLE')])});
  print(szaCollection.first());
  var szaChart = ui.Chart.image.series(szaCollection.select('SOLAR_ZENITH_ANGLE'), point, ee.Reducer.mean(), 30);
