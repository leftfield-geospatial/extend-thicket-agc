...
function findRn(image) {
  var rnImage = image.expression('(R / (R + G + B + (RE/3.5)))',
    {
      'R': image.select('B4'),
      'G': image.select('B3'),
      'B': image.select('B2'),
...
function findAgc(image) {
  var rnImage = image.expression('(R / (R + G + B + (RE/2)))',
    {
      'R': image.select('B4'),
      'G': image.select('B3'),
      'B': image.select('B2'),
...
var generateChart = function (coords) {
  ...
  var agcChart = ui.Chart.image.series(yearlyMedianImages.map(function(image){return image.select(['B2','B3','B4','B5'])}), point.buffer(60), ee.Reducer.median(), 30);
