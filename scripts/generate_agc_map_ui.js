...
var generateChart = function (coords) {
  ...
  var yearlyMedianImages = ee.ImageCollection.fromImages(years.map(function(y) {
      return l8SrImages.filter(ee.Filter.calendarRange(y, y, 'year')).filter(ee.Filter.calendarRange(10, 10, 'month'))
          .mean().set('year', y).set('system:time_start', ee.Date.fromYMD(y, 10, 15));
    }).flatten());
  print(yearlyMedianImages);
  var agcChart = ui.Chart.image.series(yearlyMedianImages.map(findAgc), point.buffer(60), ee.Reducer.mean(), 30);
...
// Point (23.94436842431511536 -33.55374308591438393) //Baviaanskloof Sewefontein
