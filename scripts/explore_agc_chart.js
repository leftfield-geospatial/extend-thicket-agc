...
function medianComp(startDate, months, coll){
  ...
  var compDate = startDate.advance(months/2, "month");
...
function dateRange(start, stop, step, units){
  ...
  return ee.List.sequence(1, numSteps).iterate(function(_, dateList){
    var nextDate = ee.Date(ee.List(dateList).get(-1)).advance(step, "months");
    return ee.List(_dateList).add(nextDate);
  }, ee.List([start]));
}
...
function addMapImageLayers(mapPanel, date){
  ...
  var compositeLayer = ui.Map.Layer(
    maskedComposite, rgbnVisParams, "RGBN (" + dateStr + ")", true, 0.6
  );
...
function createRainChart(mapPanel, toolPanel) {
  // Create rainfall time series chart
  ...
  var compList = [];
  for (var yi in years) {
    compList.push(annualMedianComp(years[yi], gpmColl));
  }
