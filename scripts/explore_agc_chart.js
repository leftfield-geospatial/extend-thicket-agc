...
var compMonths = 1;    // duration of composites in months
...
function medoidComp(startDate, months, coll){
  ...
  var _medianComp = medianComp(startDate, months); 
...
function findEvi(image) {
  ...
  return ee.Image(eviImage.log10())
  .set("system:time_start", image.get("system:time_start"))
  .rename("EVI");
}
...
function createRainChart(mapPanel, toolPanel) {
  ...
  var compList = rainDates.map(function(startDate){return meanComp(startDate, months, gpmColl)});
